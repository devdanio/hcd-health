...
function RouteComponent() {
  ...
  const markStatusMutation = useMutation({
    mutationFn: (status: "new" | "patient" | "not_patient") =>
      setLeadStatus({ data: { lead_id: leadId, status } }),
    onSuccess: async () => {
        ...
        <AppLayout orgId={orgId}>
          {leadQuery.isLoading ? (
            ...
            <LeadDetail
              ...
              setLtv={setLtv}
              setCash={setCash}
              markPatient={() => markStatusMutation.mutate("patient")}
              markNotPatient={() => markStatusMutation.mutate("not_patient")}
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={props.markPatient}>
            Mark as Patient
          </Button>
          <Button variant="outline" onClick={props.markNotPatient}>
            Not a Patient
          </Button>
        </div>
      </div>
